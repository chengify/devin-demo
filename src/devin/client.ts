...
export class DevinClient {
  ...
  async createSession(request: CreateSessionRequest): Promise<DevinSession> {
    try {
      ...
      throw new Error(
        `Failed to create Devin session: ${this.getErrorMessage(error)}`,
      );
    }
  }
  ...
  async getSession(sessionId: string): Promise<DevinSession> {
    try {
      ...
      throw new Error(
        `Failed to get Devin session: ${this.getErrorMessage(error)}`,
      );
    }
  }
  ...
  async cancelSession(sessionId: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to cancel Devin session: ${this.getErrorMessage(error)}`,
      );
    }
  }
